...
kubectl -n arc-chi run natsbox-psi --restart=Never --image=synadia/nats-box:latest -- \
  sh -lc "nats pub -s nats://chi-bus.arc-chi.svc.cluster.local:4222 omega.proposal.sandbox '{\"image\":\"ghcr.io/silentsage432/sandbox-runner:latest\",\"cmd\":[\"/bin/sh\",\"-lc\",\"echo hello from sandbox; sleep 2\"]}'"
