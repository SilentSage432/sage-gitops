  ...
  useEffect(() => {
    const refresh = async () => {
      try {
        ...
        // Fetch outcome forecast for default action
        const forecastResponse = await fetch("/api/execution/forecast?action=get-status");
        const forecastData = await forecastResponse.json();
        
        // Fetch execution chain for default action
        ...
        // Fetch execution gate preview for default action
        const gatePreviewResponse = await fetch("/api/execution/gate/preview?action=get-status");
        const gatePreviewData = await gatePreviewResponse.json();
        ...
        setChain(chainData);
        setGate(gateData);
        setGatePreview(gatePreviewData);
        setError(null);
      } catch (err) {
